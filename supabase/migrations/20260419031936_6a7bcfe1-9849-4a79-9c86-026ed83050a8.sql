...
BEGIN
  ...
  -- ===== 1) Extra sheep -> Finance + Procurement + AUTO PAYMENT REQUEST =====
  IF COALESCE(NEW.extra_sheep, 0) > 0 THEN
    ...
    -- Auto-create pending payment request in Finance (idempotent: skip if exists)
    IF finance_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.payment_requests
       WHERE committee_id = finance_id
         AND description LIKE '%[groom:' || NEW.id::text || ']%'
    ) THEN
      INSERT INTO public.payment_requests (committee_id, title, description, amount, status, requested_by)
      VALUES (
        finance_id,
        'سداد ذبائح زيادة — ' || NEW.full_name,
        'مستحق على العريس ' || NEW.full_name || ' (' || NEW.family_branch || ') · '
          || 'عدد الذبائح الزيادة: ' || NEW.extra_sheep
          || ' × 2000 ر.س = ' || extra_amount::text || ' ر.س'
          || E'\n[groom:' || NEW.id::text || ']',
        extra_amount,
        'pending',
        NEW.created_by
  ...
  -- ===== 2) External participation -> Programs =====
