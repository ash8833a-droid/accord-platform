...
-- Helper: check if user belongs to quality committee
CREATE OR REPLACE FUNCTION public.is_quality_committee_member(_user_id uuid)
RETURNS boolean
...
AS $$
  SELECT EXISTS (
    ...
    WHERE ur.user_id = _user_id AND c.type = 'quality'::committee_type
  );
$$;

-- Update RLS policies to include quality committee members
DROP POLICY IF EXISTS "Admins and women committee can view responses" ON public.women_talent_responses;
CREATE POLICY "Admins, women and quality can view responses"
...
CREATE POLICY "Admins, women and quality can update responses"
...
CREATE POLICY "Admins, women and quality can delete responses"
...
-- Notify quality committee members too on new responses
...
BEGIN
  FOR member_rec IN
    ...
    JOIN public.committees c ON c.id = ur.committee_id
    WHERE c.type IN ('women'::committee_type, 'quality'::committee_type)
  LOOP
