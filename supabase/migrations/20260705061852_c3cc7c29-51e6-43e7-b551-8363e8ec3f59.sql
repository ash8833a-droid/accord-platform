ALTER TABLE public.budget_items ADD COLUMN IF NOT EXISTS is_manual_total boolean NOT NULL DEFAULT false;
...
CREATE OR REPLACE FUNCTION public.public_add_budget_item(
  ...
  _is_manual_total boolean DEFAULT false
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.budget_items (committee_id, item_name, quantity, unit_cost, notes, assigned_by_finance, is_manual_total)
  VALUES (_committee_id, v_name, _quantity, _unit_cost,
          NULLIF(btrim(COALESCE(_notes,'')), ''), false, COALESCE(_is_manual_total, false))
