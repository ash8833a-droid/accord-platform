...
export function DashboardOverview({ showHero = true }: { showHero?: boolean }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard variant="teal" label="إجمالي الميزانية" value={`${fmt(stats.totalBudget)} ر.س`} hint={`المنصرف ${fmt(stats.spent)} ر.س`} icon={Wallet} />
        <StatCard variant="gold" label="عدد العرسان" value={fmt(stats.grooms)} hint="مسجل في النظام" icon={HeartHandshake} />
        <StatCard label="اشتراكات مؤكدة" value={fmt(stats.confirmed)} hint={`${fmt(stats.pending)} معلقة`} icon={CheckCircle2} />
        <StatCard label="نسبة إنجاز المهام" value={`${completion}%`} hint={`${stats.doneTasks} / ${stats.tasks} مهمة`} icon={TrendingUp} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <StatCard label="عدد اللجان" value={fmt(stats.committees)} hint="لجنة فعّالة" icon={Users2} />
        <StatCard label="طلبات صرف معلقة" value={fmt(stats.pendingRequests)} hint="بانتظار اعتماد المالية" icon={Receipt} />
        <StatCard label="إجمالي المهام" value={fmt(stats.tasks)} hint={`${stats.doneTasks} مكتملة`} icon={ListTodo} />
      </div>
      ...
      <div className="rounded-2xl border bg-card p-6 shadow-soft">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">أداء كل لجنة عبر الميزانية، المهام، وطلبات الصرف</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {perCommittee.map((c) => {
            ...
            const budgetPct = c.budget_allocated > 0 ? (c.budget_spent / c.budget_allocated) * 100 : 0;
              ...
              >
                ...
                <div className="space-y-1.5 mb-3">
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div className={`h-full transition-all duration-700 ${overBudget ? "bg-destructive" : "bg-gradient-gold"}`} style={{ width: `${Math.min(100, budgetPct)}%` }} />
                  </div>
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-border/60">
                  ...
                  <div className="flex items-center gap-1 ms-auto">
                    {(Object.keys(PHASE_LABELS) as PmpPhase[]).map((ph) => {
                      ...
                      return (
                        <span key={ph} className={`h-6 px-1.5 rounded-md text-[9px] font-bold inline-flex items-center ${PHASE_TONE[ph]}`} title={`${PHASE_LABELS[ph]}: ${p.done}/${p.total}`}>
          ...
          {perCommittee.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-6 col-span-full">لا توجد لجان بعد</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-gradient-to-br from-gold/10 to-transparent p-6 shadow-soft">
        ...
        <div className="space-y-2">
          ...
          <div className="h-3 bg-muted rounded-full overflow-hidden">
            <div className="h-full bg-gradient-gold transition-all duration-700" style={{ width: `${Math.min(100, (stats.confirmed / Math.max(1, stats.confirmed + stats.pending)) * 100)}%` }} />
          </div>
        </div>
      </div>
    </div>
